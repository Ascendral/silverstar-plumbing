...
(function () {
  ...
  var NAV = [
    ...
    { href: "work.html", anchor: "work", label: "Our Work" },
  ...
  /* ---------- work gallery + lightbox ----------
     Real job photos, grouped by what they show. Data comes from
     js/gallery-data.js, generated by tools/import_photos.py. */
  function renderGallery() {
    var mount = el("shots");
    var G = window.SSP_GALLERY;
    if (!mount || !G) return;

    var shown = G.photos.slice();
    var limit = parseInt(mount.getAttribute("data-limit"), 10);
    if (!isNaN(limit)) shown = shown.slice(0, limit);

    function paint(list) {
      mount.innerHTML = list.map(function (p, i) {
        return '<button class="shot" type="button" data-i="' + i + '" ' +
          'aria-label="Open photo: ' + esc(p.caption) + '">' +
          '<img src="img/gallery/t_' + p.id + '.jpg" alt="' + esc(p.caption) +
          '" loading="lazy" width="700" height="525">' +
          "<figcaption>" + esc(p.caption) + "</figcaption></button>";
      }).join("");
      Array.prototype.forEach.call(mount.querySelectorAll(".shot"), function (b) {
        b.addEventListener("click", function () { open(list, +b.getAttribute("data-i")); });
      });
    }
    ...
    if (filters) {
      var counts = {};
      G.photos.forEach(function (p) { counts[p.cat] = (counts[p.cat] || 0) + 1; });
      var btns = [{ id: "all", name: "All Work", n: G.photos.length }].concat(
        G.categories.map(function (c) { return { id: c.id, name: c.name, n: counts[c.id] || 0 }; }));
      filters.innerHTML = btns.map(function (b, i) {
        return '<button class="filter-btn' + (i === 0 ? " active" : "") + '" type="button" ' +
          'data-cat="' + b.id + '">' + esc(b.name) + '<span class="n">' + b.n + "</span></button>";
      }).join("");
      filters.addEventListener("click", function (e) {
        var b = e.target.closest(".filter-btn");
        if (!b) return;
        Array.prototype.forEach.call(filters.querySelectorAll(".filter-btn"), function (x) {
          x.classList.toggle("active", x === b);
        });
        var cat = b.getAttribute("data-cat");
        paint(cat === "all" ? G.photos : G.photos.filter(function (p) { return p.cat === cat; }));
      });
    }
    ...
    box.innerHTML =
      '<button class="lb-btn lb-close" type="button" aria-label="Close">' + svg("close") + "</button>" +
      '<button class="lb-btn lb-prev" type="button" aria-label="Previous photo">' + svg("caret") + "</button>" +
      '<button class="lb-btn lb-next" type="button" aria-label="Next photo">' + svg("caret") + "</button>" +
      '<figure><img id="lb-img" alt=""><figcaption id="lb-cap"></figcaption></figure>';
    box.querySelector(".lb-prev").style.transform = "translateY(-50%) rotate(90deg)";
    box.querySelector(".lb-next").style.transform = "translateY(-50%) rotate(-90deg)";
    document.body.appendChild(box);

    var cur = [], idx = 0, opener = null;
    function show(i) {
      idx = (i + cur.length) % cur.length;
      var p = cur[idx];
      box.querySelector("#lb-img").src = "img/gallery/" + p.id + ".jpg";
      box.querySelector("#lb-img").alt = p.caption;
      box.querySelector("#lb-cap").textContent = p.caption + " — " + p.catName;
    }
    function open(list, i) {
      cur = list; opener = document.activeElement;
      box.classList.add("open"); show(i);
